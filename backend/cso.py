...
from backend.utils import clean_key, clean_text, convert_to_int, int_or_none, utc_now_naive
...
def _event_source_probe(source: Any) -> dict[str, Any]:
    probe_details = getattr(source, "probe_details", None)
    return dict(probe_details or {})


async def _start_ffmpeg_with_hw_decode_fallback(
...
async def _terminate_ffmpeg_process(process: Any):
    if process is None:
        return
    try:
...
async def _wait_for_process_output_start(
    process: Any,
    stream: Any,
...
def _log_hwaccel_failure(policy: dict[str, Any] | None, context: str, reason: str):
...
def _resolve_cso_output_policy(policy: dict[str, Any] | None, use_slate_as_input: bool = False) -> dict[str, Any]:
...
def _resolve_vod_channel_output_policy(policy: dict[str, Any] | None, ingest_policy: dict[str, Any]) -> dict[str, Any]:
    resolved = dict(policy or {})
    resolved["subtitle_mode"] = "drop"
...
def _pipe_container_from_content_type(content_type: str) -> str:
...
def _vod_passthrough_profile_for_source(candidate: VodCuratedPlaybackCandidate | VodSourcePlaybackCandidate) -> str:
    ...
    async def start(self):
        ...
        async with self.lock:
            ...
            try:
                ...
                if self.cache_entry.complete and self.cache_entry.expected_size and self.cache_entry.final_path.exists():
...
def _effective_vod_hls_runtime_policy(policy, source: CsoSource | None):
    resolved = _effective_hls_runtime_policy(policy)
    if source is None or source.source_type not in {"vod_movie", "vod_episode"}:
        return resolved
...
class CsoIngestSession:
    ...
    async def _spawn_ingest_process(self, source_url, program_index, source: CsoSource = None):
        ...
        pipe_format = self.vod_pipe_output_format_override or _resolve_vod_pipe_container(
            source,
    ...
    async def _build_segment_runtime(self, playback, segment_index, activate_session_state=True):
        ...
        async def _attempt_start(effective_policy):
            ...
            logger.info(
                ...
                offset_seconds,
                remaining_seconds,
                _policy_log_label(effective_policy),
                _redact_ingest_command_for_log(command) if input_is_url else command,
            )
            ...
            started, startup_failure_reason, startup_chunk = await _wait_for_process_output_start(
                process,
                process.stdout,
        ...
        started, start_policy, result, _failure_reason = await _start_ffmpeg_with_hw_decode_fallback(
            base_policy,
            source_identity,
            _attempt_start,
        )
    ...
    async def start(self):
        async with self.lock:
            ...
            try:
                ...
                started, start_policy, result, failure_reason = await _start_ffmpeg_with_hw_decode_fallback(
                    self.output_policy,
                    source_identity,
                    _attempt_start,
                )
                if not started:
                    _log_hwaccel_failure(start_policy, f"output:{self.key}", failure_reason)
                    self.running = False
...
class CsoHlsOutputSession:
    ...
    async def start(self):
        async with self.lock:
            ...
            base_runtime_policy = _effective_vod_hls_runtime_policy(self.policy, source)
            self.running = True
            self.last_error = None
            self.last_activity = time.time()
            await self._prepare_output_dir()
            ...
            if started:
                ...
                return
            self.runtime_policy = dict(start_policy)
            _log_hwaccel_failure(self.runtime_policy, f"hls:{self.key}", failure_reason)
            self.running = False
            self.last_error = failure_reason or "output_start_failed"
...
async def subscribe_channel_hls(
    ...
    try:
        ingest_session.app = _current_quart_app_object()
    except Exception:
        pass
...
async def subscribe_vod_proxy_output_stream(
    ...
    policy = generate_cso_policy_from_profile(config, profile)
    source_probe = _event_source_probe(source)
    ...
    async def _attempt_start(effective_policy):
        nonlocal output_queue, should_start_cache_warm
        ...
        if using_local_cache:
            command = CsoFfmpegCommandBuilder(
                effective_policy,
                pipe_input_format=_resolve_vod_pipe_container(source, source_probe=source_probe),
                source_probe=source_probe,
            ...
            cache_meta = await _ensure_vod_cache_ready(cache_entry, request_headers=request_headers)
            ...
            command = CsoFfmpegCommandBuilder(
                ...
                max_duration_seconds=max_duration_seconds,
                input_is_url=True,
                user_agent=_resolve_cso_ingest_user_agent(config, source),
                request_headers=_resolve_cso_ingest_headers(source),
                policy=effective_policy,
            ...
            pipe_input_format = _pipe_container_from_content_type(
                proxy_session.content_type
            ) or _resolve_vod_pipe_container(
                source,
                source_probe=source_probe,
            )
            command = CsoFfmpegCommandBuilder(
        ...
        if not using_local_cache and int(start_seconds or 0) > 0:
            # Piped Matroska input is non-seekable, so output-side seeks must decode
            # forward before ffmpeg can emit the first MP4 fragment.
        ...
        if started:
            if should_start_cache_warm:
                await _start_vod_cache_download(
                    cache_entry,
                    f"{proxy_session_key}:cache-warm",
                    request_headers=request_headers,
                )
    ...
    started, start_policy, result, failure_summary = await _start_ffmpeg_with_hw_decode_fallback(
        base_policy,
        source_identity,
        _attempt_start,
    )

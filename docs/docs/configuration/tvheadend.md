...
A core feature of the recommended All-in-One (AIO) installation is the built-in, pre-configured TVHeadend server. While Headendarr manages the channels and EPG, TVHeadend does the heavy lifting of streaming, DVR, and client management.
...
By integrating TVHeadend, Headendarr can offer a robust and reliable backend for delivering your IPTV streams.

## How Headendarr uses TVHeadend

When you use the AIO installation, Headendarr automatically configures and manages the internal TVHeadend instance:

-   **Channel & EPG Sync**: When you map channels and sync them in Headendarr, it automatically creates the necessary configuration in TVHeadend.
...
Some TVHeadend settings are actively managed and overwritten by Headendarr. Specifically:
*   Any networks prefixed with `tic-` will be managed by Headendarr.
*   Any Muxes prefixed with `tic-` will be managed by Headendarr.
*   Streaming profiles and recording profiles prefixed with usernames are managed by Headendarr.

Changes made to these specific items directly within the TVHeadend UI may be overwritten by Headendarr to maintain consistency and proper operation.
:::

### What can you do in the TVHeadend UI?

For the most part, you should not need to change settings in the TVHeadend UI directly, as Headendarr will manage the important parts for you. However, it can be useful for:
...
-   **Advanced EPG/DVR Settings**: Accessing more granular settings for the EPG and DVR that are not exposed in the Headendarr interface.
-   **Creating Client Users**: Creating specific user accounts for client applications with different levels of access.

:::warning
It is recommended to let Headendarr manage the core channel, mux, and EPG configuration. Manually changing these settings in the TVHeadend UI can cause conflicts with the settings synced from Headendarr.
:::

...
- **Retention Policy**: (Available if DVR Access is granted) A dropdown to select how long recordings are retained for this user before automatic deletion.
  See [Retention Policy Values](/configuration/application-settings/#retention-policy-values) for what each option means and how it affects cleanup behaviour.
